...
- `Note Relation Agent` 只提供程序性知识线索
- `Doc Maintenance Agent` 才负责第一次正式判断：更新后的 doc 是否已经形成真实的 SOP promotion signal
  ...
  "sop_signals": [
    {
      ...
      "reason": "程序性知识已出现，但仍需更多 case 验证触发条件与结束条件",
      "steps_evidence": [
        "已能抽出相对稳定的处理顺序"
      ],
      "open_questions": [
        "该流程是否只适用于当前 task family 的子分布"
      ]
    }
  ],
...
其中 `sop_signals` 建议始终显式出现：

- 有 signal 时写对象数组
- 没有 signal 时写空数组 `[]`

而且这里的 `sop_signals` 才是 Agent3 的**主输入信号**；Agent1 的 `procedural_hints` 只应作为辅助证据链，而不是越级裁决。
...
- 本次更新后哪些 doc 已经形成真实的 SOP promotion signal
