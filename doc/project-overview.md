...
这个设计与 meta-agent 方法论天然契合：daily notes 的积累机制就是 experience buffer 的实现。
...
| 层次 | RL 含义 | 信息粒度 |
|------|---------|----------|
| Daily notes | Task-level 经验 | 单次 rollout 的细节 |
| Doc | Expert-level 知识 | 多次 task 的并集，去掉 task-specific 细节 |
| SOP | 被验证的可复用策略 | 从 doc 中进一步结晶的操作流程 |
